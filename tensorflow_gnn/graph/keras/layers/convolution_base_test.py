...
# From AnyToAnyConvolutionBase.__init__.__doc__, except symbol `tfgnn.`.
def ExampleEdgePool(*args, sender_feature=const.DEFAULT_STATE_NAME, **kwargs):  # To be called like a class initializer.  pylint: disable=invalid-name
...
class AnyToAnyConvolutionBaseTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def testExampleNodeToNode(self, use_sender_edge_input, use_receiver_input,
    ...
    if use_sender_edge_input:
      conv_kwargs["sender_edge_feature"] = const.DEFAULT_STATE_NAME
  ...
  def testExampleEndToEnd(self, reload_model):
    ...
    self.assertAllEqual(
        [[1. + 4. + 0.5], [2. + 4. + 0.5], [0.]],
        example_output.node_sets["nodes"][const.DEFAULT_STATE_NAME])
    self.assertAllEqual(
        [[(5.5 + 100*1) + (6.5 + 100*1) + (0. + 100*1)]],
        example_output.context[const.DEFAULT_STATE_NAME])
