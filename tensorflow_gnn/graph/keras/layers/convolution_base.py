...
class AnyToAnyConvolutionBase(tf.keras.layers.Layer, abc.ABC):
  ...
  `sender_node_feature=tfgnn.DEFAULT_STATE_NAME` and `sender_edge_feature=None`.
  ...
  def call(self, graph: gt.GraphTensor, *,
    ...
    if receiver_tag is None:
      if self._receiver_tag is None:
        raise ValueError(f"{class_name} requires receiver_tag to be set "
                         "at init or call time")
      receiver_tag = self._receiver_tag
    else:
      if self._receiver_tag not in [None, receiver_tag]:
        raise ValueError(
            f"{class_name}(..., receiver_tag={self._receiver_tag})"
